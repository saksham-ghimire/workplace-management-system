...
class Elasticsearch:
    ...
    def __get(self,query) -> list:
        try:
            response = requests.get(**query,**self.__generic_query_parameters,timeout=8)
            if response.status_code != 200:
                log.debug(f'Elasticsearch responded with {response.content}')
                return None
            resp = json.loads(response.content)
            if resp.get("aggregations") == None: return [i.get("_source") for i in resp.get("hits").get("hits")]
            return resp.get("aggregations").get("categories_agg").get("buckets")
           
        
        except (ConnectionError,Timeout) as e:
            log.debug(f'Unable to perform get opertation. Received exception : {e}')
            return None

    def __post(self,query):
        try:
            response = requests.post(**query,**self.__generic_query_parameters,timeout=8)
            if response.status_code != 201:
                log.debug(f'Elasticsearch responded with {response.content}{response.status_code}')
                return None
            return True
        except (ConnectionError,Timeout) as e:
            log.debug(f'Unable to perform post operation. Received exception : {e}')
            return None
    ...
    def getWorkstationInfo(self, hostname:str) -> dict:
        response = self.__get(query=QuerySerializer.getDesignatedQuery(reqType=SupportedQueries.GetWorkstationInfo,params={"hostname":hostname}))
        if response != None or len(response)!=0: return response[0] 
        return None
    
    def getRegisteredWorkstations(self) -> 'list[dict]':
        response = self.__get(query=QuerySerializer.getDesignatedQuery(reqType=SupportedQueries.GetRegisteredWorkstations,params={}))
    ...
    def pushProcessRuntimeLog(self,hostname:str,data:'list[str]'):
        for eachLog in data:
            self.__post(query=QuerySerializer.getDesignatedQuery(reqType=SupportedQueries.PostRunningProcesses,params={"data":{"processName":eachLog},"hostname":hostname}))

    def pushMonitoringProcessLog(self, hostname:str,data:dict):
        for eachLog in data:
            self.__post(query=QuerySerializer.getDesignatedQuery(reqType=SupportedQueries.PostMonitoringProcessDetail,params={"data":eachLog,"hostname":hostname}))
