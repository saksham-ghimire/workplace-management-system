...
def initializeAvailableWorkstations():
    ...
    for workstation in workstations:
        w = WorkStation(
            ...
            blockingProcesses=workstation.get("restrictedProcesses")
        )
    ...
    while True:
        ...
        for workstation in workstations.values():
            ...
            if runningProcesses != None: 
                elasticInstance.pushProcessRuntimeLog(hostname=workstation.hostname, data=runningProcesses)
                breaches = set(workstation.blockingProcesses).intersection(runningProcesses)
                if breaches != set():
                    for i in breaches:
                        req = structure_pb2.StreamRequest(MessageType="Warning",Message=f'restriced application {i} has been detected running, this incident will be reported')
